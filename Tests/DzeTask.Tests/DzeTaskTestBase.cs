...
namespace DzeTask.Tests
{
    public abstract class DzeTaskTestBase : AbpIntegratedTestBase<DzeTaskTestModule>
    {
        ...
        protected void UsingDbContext(Action<DzeTaskDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<DzeTaskDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<DzeTaskDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<DzeTaskDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<DzeTaskDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DzeTaskDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<DzeTaskDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<DzeTaskDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<DzeTaskDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<DzeTaskDbContext, Task<T>> func)
        {
            T result;
